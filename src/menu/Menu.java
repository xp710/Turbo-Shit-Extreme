...
public class Menu extends DefaultHook {
	...
	try {
	    ap = new AudioPlayer("/music/title.wav", true);
	    ap.play();
	}catch (Exception e){
    ...
    private void drawTitle(Graphics2D graphics) {
        String text1 = "TURBO SHIT XTREME!";
        String text2 = "A mod of Starlorn by xp710. Starlorn by Josh Hofing, Ben Kurtovic, and Victor Jiao";
	String text3 = "Music by MusMus";
        int xOffset1 = screen.getXOffset(graphics, bigFont, text1);
        int xOffset2 = screen.getXOffset(graphics, smallFont, text2);
	int xOffset3 = screen.getXOffset(graphics, smallFont, text3);
	...
	graphics.drawString(text3, xOffset3, screen.getHeight() / 2 - 180);
    }
